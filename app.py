...
def execute(sql: str, params=None):
    """Run a write query. Returns True on success."""
    try:
...
def init_db():
    ...
    # Seed admin if not exists (password stored hashed)
    seed = """
...
# SESSION & AUTH
# ══════════════════════════════════════════════════════════════
_defaults = {"auth": False, "role": None, "user": None, "ref": None}
    ...
    with col2:
        ...
        if submitted:
            if not username or not password:
                ...
                users = run_query(
                    "SELECT * FROM public.system_users WHERE username = :u AND password = :p",
                    {"u": username, "p": hash_password(password)},
                )
                # Fallback: check plain-text password (migration support)
                if users.empty:
                    users = run_query(
                        "SELECT * FROM public.system_users WHERE username = :u AND password = :p",
...
if menu == "Dashboard":
    ...
    clients  = run_query("SELECT * FROM public.clients")
    ...
    c1.metric("Total Contracts",  fmt_dh(total_budget))
    c2.metric("Cash Collected",   fmt_dh(total_advance),
              f"{pct(total_advance, total_budget):.0f}% of total")
    c3.metric("Labor Cost",       fmt_dh(total_labor))
    c4.metric("Materials",        fmt_dh(total_mat))
    c5.metric("Gross Profit",     fmt_dh(gross_profit),
...
# ══════════════════════════════════════════════════════════════
    ...
    with col_form:
        with st.form("estimator_form"):
            ...
            c1, c2 = st.columns(2)
            points = c1.number_input("Estimated Points", min_value=1, value=None, placeholder="0")
            surface = c2.number_input("Surface m² (optional)", min_value=0, value=None, placeholder="0")
    ...
    with col_result:
        if submitted:
            if not project_name or not points:
                st.error("Project name and points are required.")
            else:
                ...
                if st.button("📄 Export Quote PDF", type="primary", use_container_width=True):
                    ...
                    pdf.cell(0, 8, f"Projet: {project_name}", ln=True)
                    pdf.cell(0, 8, f"Points: {points} | Marge: {margin_pct}%", ln=True)
                    pdf.ln(4)
                    pdf.set_font("Arial", "B", 14)
                    pdf.cell(0, 10, f"Prix recommande: {fmt_dh(quote)}", ln=True)
                    pdf.set_font("Arial", "", 11)
                    for k, v in details.items():
                        pdf.cell(0, 7, f"  {k}: {v}", ln=True)
                    fname = f"devis_{project_name.replace(' ','_')}.pdf"
                    pdf.output(fname)
...
# ══════════════════════════════════════════════════════════════
    ...
    clients = run_query("SELECT * FROM public.clients ORDER BY id")

    tab_list, tab_add, tab_payment = st.tabs(["📋 Sites List", "➕ New Site", "💰 Log Payment"])
    ...
    with tab_list:
        if clients.empty:
            ...
            for _, r in clients.iterrows():
                ...
                if col_btn.button("Mark Done", key=f"done_{r['id']}", disabled=(status == "completed")):
    ...
    with tab_add:
        with st.form("new_client_form"):
            ...
            wt   = c2.selectbox("Work Type", ["Full Electrical", "Tableau", "Tirage", "Appareillage", "VDI", "Domotique"])
            bg   = c1.number_input("Budget (DH) *", min_value=0.0, value=None, placeholder="0.0")
            pts  = c2.number_input("Total Points", min_value=0.0, value=None, placeholder="0")
            adv  = c1.number_input("Initial Advance (DH)", min_value=0.0, value=None, placeholder="0.0")
            if st.form_submit_button("✅ Create Site", type="primary"):
                if not nm or not bg:
                    st.error("Name and budget are required.")
                elif run_query("SELECT 1 FROM public.clients WHERE client_name=%s", (nm,)).empty is False:
                    ...
                    ok = execute(
                        ...
                        (nm, wt, bg or 0, adv or 0, pts or 0),
                    )
                    if ok:
    ...
    with tab_payment:
        if clients.empty:
            ...
            with st.form("payment_form"):
                ...
                c1, c2 = st.columns(2)
                amt    = c1.number_input("Amount Received (DH)", min_value=0.01, value=None, placeholder="0.0")
                method = c2.selectbox("Payment Method", ["Virement bancaire", "Chèque", "Espèces", "Autre"])
...
# ══════════════════════════════════════════════════════════════
    ...
    workers = run_query("SELECT * FROM public.workers ORDER BY name")
    clients = run_query("SELECT client_name FROM public.clients WHERE status='active'")
    ...
    with col_form:
        with st.form("timesheet_form"):
            ...
            site  = st.selectbox("Site", clients["client_name"] if not clients.empty else ["—"])
            ...
            for _, w in workers.iterrows():
                ...
                days_val = cols[1].number_input("d", min_value=0.0, max_value=30.0, step=0.5,
                                                 value=None, placeholder="0", key=f"wdays_{w['id']}",
                                                 label_visibility="collapsed")
                if checked:
                    worker_days[w["name"]] = (days_val or 0, w["tjm"])

            submitted = st.form_submit_button("✅ Submit Logs", type="primary", use_container_width=True)

        if submitted:
            if not worker_days:
                st.error("Select at least one technician.")
            elif all(v[0] == 0 for v in worker_days.values()):
                st.error("Enter days for at least one worker.")
            else:
                logged = 0
    ...
    with col_log:
        ...
        logs = run_query(
            "SELECT date, worker_name, client_name, phase, days, cost FROM public.labor_logs ORDER BY id DESC LIMIT 30"
        )
        if not logs.empty:
            logs["cost"] = logs["cost"].apply(fmt_dh)
            logs.columns = ["Date", "Worker", "Site", "Phase", "Days", "Cost"]
...
# ══════════════════════════════════════════════════════════════
    ...
    workers    = run_query("SELECT name FROM public.workers ORDER BY name")
    worker_filter = col3.selectbox("Worker", ["All Workers"] + workers["name"].tolist() if not workers.empty else ["All Workers"])
...
# ══════════════════════════════════════════════════════════════
    ...
    clients = run_query("SELECT client_name FROM public.clients ORDER BY client_name")
    ...
    with col_form:
        with st.form("expense_form"):
            ...
            c1, c2 = st.columns(2)
            site     = c1.selectbox("Site", clients["client_name"] if not clients.empty else ["—"])
            phase    = c2.selectbox("Phase", PHASES)
            item     = st.text_input("Item Description *", placeholder="e.g. Câbles 2.5mm² (500m)")
            c1b, c2b = st.columns(2)
            amt      = c1b.number_input("Amount (DH) *", min_value=0.01, value=None, placeholder="0.0")
            ...
            if st.form_submit_button("+ Log Expense", type="primary"):
                if not item or not amt:
                    st.error("Item description and amount are required.")
                else:
                    ok = execute(
    ...
    expenses = run_query("SELECT date, client_name, phase, item, amount, supplier FROM public.expenses ORDER BY id DESC")
    if not expenses.empty:
        expenses["amount"] = expenses["amount"].apply(fmt_dh)
        expenses.columns = ["Date", "Site", "Phase", "Item", "Amount", "Supplier"]
...
# ══════════════════════════════════════════════════════════════
    ...
    if not inventory.empty:
        low = inventory[inventory["quantity"] < 20]
        if not low.empty:
    ...
    with col_in:
        with st.form("checkin_form"):
            ...
            qty_in   = c2.number_input("Quantity", min_value=0.1, value=None, placeholder="0")
            c1b, c2b = st.columns(2)
            unit_in  = c1b.selectbox("Unit", ["pcs", "m", "kg", "boîte", "rouleau"])
            cat_in   = c2b.selectbox("Category", ["Câblage", "Protection", "Appareillage", "Conduit", "Tableau", "Autre"])
            if st.form_submit_button("+ Receive Stock", type="primary", use_container_width=True):
                if not item_in or not qty_in:
                    st.error("Item and quantity required.")
                else:
                    execute(
                        """INSERT INTO public.inventory (item_name,category,quantity,unit)
                           VALUES (%s,%s,%s,%s)
                           ON CONFLICT (item_name) DO UPDATE
                           SET quantity = inventory.quantity + EXCLUDED.quantity""",
                        (item_in, cat_in, qty_in, unit_in),
                    )
                    execute(
    ...
    with col_out:
        with st.form("checkout_form"):
            ...
            qty_out   = c2.number_input("Quantity", min_value=0.1, value=None, placeholder="0", key="qout")
            clients   = run_query("SELECT client_name FROM public.clients ORDER BY client_name")
            ...
            if st.form_submit_button("- Issue Stock", type="primary", use_container_width=True):
                if not items_list or not qty_out:
                    st.error("Item and quantity required.")
                else:
    ...
    if not inventory.empty:
        def status_tag(q):
            if q < 20:   return "🔴 Critical"
            if q < 50:   return "🟡 Low"
            return "🟢 OK"
        inventory["Status"] = inventory["quantity"].apply(status_tag)
        st.dataframe(
            inventory[["item_name","category","quantity","unit","Status"]]
            .rename(columns={"item_name":"Item","category":"Category","quantity":"Qty","unit":"Unit"}),
...
# ══════════════════════════════════════════════════════════════
    ...
    clients = run_query("SELECT * FROM public.clients ORDER BY client_name")
    if clients.empty:
        st.info("No clients yet.")
        st.stop()
    ...
    with col_inv:
        ...
        if col_a.button("📄 Export PDF Invoice", type="primary", use_container_width=True):
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial","B",18)
            ...
            pdf.output(fname)
            with open(fname,"rb") as f:
                col_a.download_button("⬇ Download", f.read(), fname, mime="application/pdf")

    with col_hist:
        st.subheader("Quick Payment")
        with st.form("quick_pay_form"):
            pay_amt    = st.number_input("Amount (DH)", min_value=0.01, value=None, placeholder="0.0")
            pay_method = st.selectbox("Method", ["Virement bancaire","Chèque","Espèces","Autre"])
...
# ══════════════════════════════════════════════════════════════
    ...
    workers = run_query("SELECT * FROM public.workers ORDER BY name")
    clients = run_query("SELECT client_name FROM public.clients WHERE status='active' ORDER BY client_name")
...
# ══════════════════════════════════════════════════════════════
    ...
    tab_users, tab_workers, tab_security = st.tabs(["Users", "Workers", "Security"])
    ...
    with tab_users:
        ...
        with col_form:
            with st.form("add_user_form"):
                ...
                u_role = st.selectbox("Role", ["Admin", "Technician", "Client"])
        ...
        with col_list:
            ...
            users = run_query("SELECT username, role, reference FROM public.system_users ORDER BY username")
            if not users.empty:
                st.dataframe(users.rename(columns={"username":"Username","role":"Role","reference":"Reference"}),
                             use_container_width=True, hide_index=True)
    ...
    with tab_workers:
        ...
        with col_form:
            with st.form("add_worker_form"):
                ...
                c1, c2 = st.columns(2)
                w_tjm  = c1.number_input("Daily Rate (DH/j)", min_value=1.0, value=None, placeholder="500")
                w_spec = c2.selectbox("Specialty", PHASES)
                if st.form_submit_button("Add Technician", type="primary"):
                    if not w_name or not w_tjm:
        ...
        with col_list:
            ...
            workers = run_query("SELECT name, specialty, tjm FROM public.workers ORDER BY name")
            if not workers.empty:
                workers["tjm"] = workers["tjm"].apply(fmt_dh)
                workers.columns = ["Name", "Specialty", "Daily Rate"]
    ...
    if not photos.empty:
        ...
        for idx, (_, row) in enumerate(photos.iterrows()):
            try:
                ...
                st.warning(f"Could not load photo #{row.get('id','?')}: {e}")
